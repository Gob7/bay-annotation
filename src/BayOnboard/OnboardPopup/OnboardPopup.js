...
function OnboardPopup() {
  ...
  const handleDone = (e, bayId) => {
    if (drawingData.length > 0) {
      ...
      const polygon = {
        ...
        actualArea:
          (ACTUAL_BLUSHLACE_AREA / plottedDimensions.sectionArea) * plottedArea,
      };
  ...
  const cellSize = Math.round(
    Math.sqrt(
      realGridArea * (sectionProps.plottedArea / sectionProps.actualArea)
    )
  ); // Adjust the grid cell size as needed
  ...
  useEffect(() => {
    ...
    setPlottedDimensions((prevState) => ({
      ...prevState,
      sectionArea: sectionProps.plottedArea,
    }));
    setRealDimension((prevState) => ({
      ...prevState,
      sectionArea:
        ((realDimension.height * realDimension.width) /
          (plottedDimensions.height * plottedDimensions.width)) *
        sectionProps.plottedArea,
    }));
  }, [sectionProps]);
                        ...
                        <g>
                          <defs>
                            <clipPath id={`clip-path`}>
                              <polygon
                                fill="rgba(250, 125, 50, 0.3)"
                                points={polygonPoints}
                              />
                            </clipPath>
                          </defs>
                          <g clipPath={`url(#clip-path`}>{lines}</g>
                        </g>
                      </svg>
                    )}
                  </svg>
                  ...
                  {boxProps.map((box) => (
                    <div
                      ...
                      className={`absolute border-2 border-${
                        box.id === annotatingBayId ? "green-500" : "black"
                      } cursor-${
                        annotatingBayId === box.id ? "pointer" : "not-allowed"
                      } ${
                        hoveredBoxId === box.id
